...
const TestItem = () => {
  ...
  const [answerHandler, updateAnswerHandler] = useState(
    test.tests.map((el) => ({type: el.content.type, selected: null})),
  );
  const [currentTest, setCurrentTest] = useState(0);
  const isLastTest = useMemo(() => currentTest + 1 > test?.tests?.length - 1, [
    currentTest,
  ]);
  ...
  return (
    <View>
      {!showResults && (
        <>
          <CustomHeader title={test.title} canGoBack isTest />
          <View style={styles.mainTestBody}>
            <View style={sequenceContainer}>
              <Text>
                {`${currentTest + 1}`}/{test?.tests?.length?.toString()}
              </Text>
            ...
            <ScrollView style={{height: '65%', overflow: 'scroll'}}>
              <TestPicker
                content={test.tests[currentTest].content}
                answerHandler={answerHandler[currentTest]}
            ...
            <View style={{alignSelf: 'center', paddingTop: 10}}>
              <Button
                title={isLastTest ? 'Завершити' : 'Далі'}
        ...
        <>
          <View style={styles.resultsBlock}>
            <View style={styles.resultUppperContent}>
              ...
              <Text style={styles.upperText} adjustsFontSizeToFit>
                {test.title}
              </Text>
            ...
            <View style={styles.resultContent}>
              ...
              <Text
                style={
                  styles.yourScore
                }>{`${finalResults.score}/${finalResults.maxScore}`}</Text>
            </View>
            <View style={styles.resultsStats}>
              ...
              <View style={styles.resultsStatsItem}>
                ...
                <Text style={styles.statItemScore}>
          ...
          <View style={styles.explanationBlock}>
            <FlatList
              ...
              renderItem={({item, index}) => (
                <TestResutItem
                  originalTest={item}
                  answeredTest={answerHandler[index]}
                  index={index}
                />
              )}
            />
