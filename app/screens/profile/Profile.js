...
const Profile = () => {
    ...
    const { container, iconsBlock, profileHeader, userInfo, userName, userPhoneNumber, imageBlock } = styles
    const { navigate } = useNavigation()
    const { username, phone, avatar } = useSelector(state => state.auth)
    React.useEffect(
        () => {
            getUserData().then(
                res => console.log('result getting userData', res)
            )
                .catch(
    ...
    return (
        <View style={container}>
            <View style={profileHeader}>
                ...
                <View style={userInfo}>
                    {
                        avatar?.length > 0 && <View style={imageBlock}>
