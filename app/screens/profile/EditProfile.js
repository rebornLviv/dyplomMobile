...
const EditProfile = () => {
  ...
  const updateProfile = () => {
    try {
      ...
      changeUserData({avatar: userImage, email: newEmail})
  ...
  const hasChanges = useMemo(() => newEmail !== email || avatar !== userImage, [
    email,
    newEmail,
    avatar,
  ]);
  const getNewPhotoFromLibrary = async () => {
    try {
      await launchImageLibrary(
        {includeBase64: true, quality: 0.5, mediaType: 'photo'},
        (response) => {
    ...
    <KeyboardAvoidingView style={styles.container}>
      ...
      <ScrollView
        ...
        <TouchableOpacity
          ...
          onPress={getNewPhotoFromLibrary}>
          {!userImage?.length > 0 ? (
            <PhotoPlaceholderItem containerStyles={{height: 120, width: 120}} />
          ) : (
            <Image
        ...
        <Button
          btnStyle={{
            marginVertical: 20,
            backgroundColor: hasChanges ? Colors.main : Colors.gray,
          }}
